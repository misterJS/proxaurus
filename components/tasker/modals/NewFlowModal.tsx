...
import { insertFlow } from '@/services/tasker/flows';
...
export default function NewFlowModal({
    ...
    nextPosition: number;
    onClose: () => void;
    onCreated?: (flow: { id: string; name: string; position: number }) => void;
}) {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        setErr(null);
        const { data, error } = await insertFlow(projectId, name.trim(), nextPosition);
        setLoading(false);
        if (error) {
    ...
    return (
        <ModalShell title="Flow baru" onClose={onClose} maxWidth="max-w-md">
            <form className="grid gap-4" onSubmit={submit}>
                ...
                <label className="grid gap-1 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                    Nama flow
                    <input type="text" className="form-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Misal: Backlog" required />
                </label>
                <div className="flex items-center justify-end gap-3 pt-2">
                    ...
                    >
                        <IconPlus className="h-4 w-4" /> {loading ? 'Menyimpan...' : 'Buat flow'}
                    </button>
                </div>
            </form>
        </ModalShell>
